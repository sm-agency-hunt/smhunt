...
class GroqProvider(AIProvider):
    ...
    async def classify_intent(self, text: str, **kwargs) -> str:
        ...
        categories = ["interested", "pricing_question", "request_meeting", "not_interested", "needs_followup", "unsubscribe"]
        prompt = f"Classify the intent. Categories: {', '.join(categories)}. Text: {text}. Intent:"
    ...
    async def analyze_sentiment(self, text: str, **kwargs) -> Dict[str, float]:
        ...
        prompt = f"Analyze sentiment of: {text}. Return JSON: {{positive: score, negative: score, neutral: score}}"
...
class MockAIProvider(MockProvider, AIProvider):
    ...
    async def generate_content(
        ...
        **kwargs
        ...
        if "outreach" in prompt.lower() or "email" in prompt.lower():
            return f"Mock personalized outreach email content based on prompt: {prompt[:50]}..."
    ...
    async def classify_intent(self, text: str, **kwargs) -> str:
        ...
        import random
        intents = ["interested", "pricing_question", "request_meeting", "not_interested", "needs_followup"]
